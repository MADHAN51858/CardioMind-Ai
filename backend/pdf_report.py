...
def generate_pdf_report(patient_data, prediction_info, metrics_summary, dest_path):
    ...
    cp_display = cp_mapping.get(cp_val, patient_data.get("cp_label", str(cp_val)))
    sex_display = "Male" if patient_data.get("sex") == 1 else "Female"
    fbs_display = "True (>120 mg/dl)" if patient_data.get("fbs") == 1 else "False (<=120 mg/dl)"
    exang_display = "Yes (Exertional pain)" if patient_data.get("exang") == 1 else "No"
    
    flat_params = [
        ("Age", f"{patient_data.get('age')} years", "Sex", sex_display),
        ("Chest Pain Experience", cp_display, "Resting Blood Pressure", f"{patient_data.get('trestbps')} mm Hg"),
        ("Serum Cholesterol", f"{patient_data.get('chol')} mg/dl", "Fasting Blood Sugar", fbs_display),
        ("Exercise-Induced Angina", exang_display, "Heart Rate / Pulse", f"{patient_data.get('thalach')} bpm")
    ]
    
    for row in flat_params:
        param_data.append([
            Paragraph(row[0], table_cell_style), Paragraph(row[1], table_cell_style),
            Paragraph(row[2], table_cell_style), Paragraph(row[3], table_cell_style)
        ])
    ...
    prob = prediction_info.get("probability", 0.0) * 100
    category = prediction_info.get("category", "N/A")
    model_name = prediction_info.get("model", "XGBoost")
    ...
    for item in prediction_info.get("top_positive_features", [])[:3]:
        feat = item["feature"].replace("__", " ").replace("num_", "").replace("cat_", "")
        shap_data.append([
            Paragraph(feat.upper(), table_cell_style),
            Paragraph(f"+{item['shap_value']:.4f}", table_cell_style),
